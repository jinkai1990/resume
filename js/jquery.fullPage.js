...
(function($) {
	$.fn.fullpage = function(options) {
		...
		*/
		...
		//首页第一屏 跳转其他屏次 .help_link_list   .help_link_sub
		$(document).on('click touchstart', '.help_link_list .help_link_sub', function(e){
			e.preventDefault();
			var index = $(".help_link_list>.help_link_sub").index(this);
			scrollPage($('.fp-section').eq(index+1));
		});
		
		//#fp_nav  .fp_point 此导航已失效 2016.05.07
		$(document).on('click touchstart', '#fp_nav .fp_point', function(e){
			e.preventDefault();
			var index = $("#fp_nav>.fp_point").index(this);
			scrollPage($('.fp-section').eq(index));
		});
		
		//#fr_nav  .fr_navcell
		$(document).on('click touchstart', '.tab_ul li', function(e){
			e.preventDefault();
		...
		*/
		function setBodyClass(text){
			...
			//text = text.replace('/', '-').replace('#','');    /*0419*/

			//removing previous anchor classes
			//$("body")[0].className = $("body")[0].className.replace(/\b\s?fp-viewing-[^\s]+\b/g, '');/*0419*/

			//adding the current anchor
			//$("body").addClass("fp-viewing-" + text);/*0419*/
		}
